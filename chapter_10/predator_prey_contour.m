...
%%%%% CODE FOR PREDATOR_PREY CONTOUR EXAMPLE (SECTION 10.3.4) %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n = nquad
    ...
    hleft = cell(1,1);
    hleft{1,1} = @(z) (z-Z0).^0;
    [V,zpts,hcn(ct,1)] = Hankel_cont(M,Mv,Z0,quadpts,totwts,hleft,hleft,mreal);
    hres(ct,1) = max(pred_prey_residual(LI,Ld,L1,L2a,L2b,V,zpts));

    hleft{2,1} = @(z) (z-Z0).^1;
    [V,zpts,hcn(ct,2)] = Hankel_cont(M,Mv,Z0,quadpts,totwts,hleft,hleft,mreal);
    hres(ct,2) = max(pred_prey_residual(LI,Ld,L1,L2a,L2b,V,zpts));

    hleft{3,1} = @(z) (z-Z0).^3;
    [V,zpts,hcn(ct,3)] = Hankel_cont(M,Mv,Z0,quadpts,totwts,hleft,hleft,mreal);
    hres(ct,3) = max(pred_prey_residual(LI,Ld,L1,L2a,L2b,V,zpts));

    hleft{4,1} = @(z) (z-Z0).^4;
    [V,zpts,hcn(ct,4)] = Hankel_cont(M,Mv,Z0,quadpts,totwts,hleft,hleft,mreal);
    hres(ct,4) = max(pred_prey_residual(LI,Ld,L1,L2a,L2b,V,zpts));

        
    [V,zpts,lcn(ct)] = Loewner_cont(M,Mv,Z0,quadpts,totwts,zleft,zright,mreal);
    lres(ct) = max(pred_prey_residual(LI,Ld,L1,L2a,L2b,V,zpts));

       
    semilogy(nquad(1:ct),hres(1:ct,:));
    hold on
    semilogy(nquad(1:ct),lres(1:ct),'k');
    hold off
    pause(0.01)
    ct = ct + 1;
end
...
legend({'$u^*(x)$ (prey)','$v^*(x)$ (predator)'},'fontsize',16,'interpreter','latex','location','best')

figure
plot(real(zpts),imag(zpts),'.k','markersize',16)
hold on
plot(real(contour(0:0.01:2*pi)),imag(contour(0:0.01:2*pi)),':k','linewidth',2)
ax=gca; ax.FontSize=18;
ylabel('$\mathrm{Im}(z)$','interpreter','latex','fontsize',18)
xlabel('$\mathrm{Re}(z)$','interpreter','latex','fontsize',18)
legend({'eigenvalues','contour'},'fontsize',18,'interpreter','latex','location','east')
box on
set(gca,'layer','top');
hold on
axis equal
...
figure
plot(real(zleft),imag(zleft),'*k','markersize',3)
hold on
plot(real(zright),imag(zright),'ok','markersize',5)
plot(real(contour(0:0.01:2*pi)),imag(contour(0:0.01:2*pi)),':k','linewidth',2)
ax=gca; ax.FontSize=18;
ylabel('$\mathrm{Im}(z)$','interpreter','latex','fontsize',18)
xlabel('$\mathrm{Re}(z)$','interpreter','latex','fontsize',18)
legend({'$z_{\mathrm{left}}$','$z_{\mathrm{right}}$'},'fontsize',18,'interpreter','latex','location','eastoutside')
box on
set(gca,'layer','top');
hold on
axis equal
...
figure
semilogy(nquad,hcn(:,1),'k','linewidth',1);
hold on
semilogy(nquad,hcn(:,2),'--k','linewidth',2);
semilogy(nquad,hcn(:,3),':k','linewidth',2);
semilogy(nquad,hcn(:,4),'-.k','linewidth',2);
semilogy(nquad(1:length(lcn)),lcn,'k','linewidth',3);
ax=gca; ax.FontSize=16;
ylim([1000,10^17])
xlabel('number of quadrature points','interpreter','latex','fontsize',18)
title('Condition Number of $\tilde{B}_0$','interpreter','latex','fontsize',18)
legend('Hankel, $p=1$','Hankel, $p=2$','Hankel, $p=3$','Hankel, $p=4$','Multipoint Loewner','interpreter','latex','fontsize',12,'location','best')
grid on

figure
semilogy(nquad,hres(:,1),'k','linewidth',1);
hold on
semilogy(nquad,hres(:,2),'--k','linewidth',2);
semilogy(nquad,hres(:,3),':k','linewidth',2);
semilogy(nquad,hres(:,4),'-.k','linewidth',2);
semilogy(nquad(1:length(lcn)),lres,'k','linewidth',3);
ax=gca; ax.FontSize=16;
ylim([10^(-10),10^7])
xlabel('number of quadrature points','interpreter','latex','fontsize',18)
title('Max Eigenpair Residual','interpreter','latex','fontsize',18)
legend('Hankel, $p=1$','Hankel, $p=2$','Hankel, $p=3$','Hankel, $p=4$','Multipoint Loewner','interpreter','latex','fontsize',12,'location','southwest')
grid on

 function [L, loc, edges, indx_bdy] = circ_lap_dual_rhomb(level)

% function [L, loc, edges, indx_bdy] = circ_lap_dual_rhomb(level)
%
% Construct the Robinson stone version of the Penrose tiling,
% then remove edges to give a rhombus tiling.               
%
% Embree and Fillman, 2015-2023
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 keep = find(abs(abs(loc(edges(:,1))-loc(edges(:,2)))-(sqrt(5)-1)/2)<1e-8);
 edges = edges(keep,:);
 L = sparse(edges(:,1),edges(:,2),ones(size(edges,1),1),dN,dN,dN+4*N);
 L = L+L';

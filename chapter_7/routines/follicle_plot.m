 function fig = follicle_plot(sigma, lamvec, plot_bdy, lw, col)
...
% the lines used to draw the intervals.
 ...
 dlam = 1/(2*q);
 for j=0:q
    S = sigma{j+1};                % compute the itervals
    ...
    if j>0
        plot(real(Sp),imag(Sp)*lamvec(end),'-','linewidth',lw,'color',col)
    else
        plot(real(Sp),(imag(Sp)+1/(4*q))*lamvec(end),'-','linewidth',lw/2,'color',col)
    ...
    if plot_bdy>0
       ...
       plot(real(Sr),imag(Sr)*lamvec(end),'k-','linewidth',.1)
    end
 end

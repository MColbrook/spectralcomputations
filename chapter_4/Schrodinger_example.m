...
%%%%%%%%% CODE FOR REAL LINE SCHRODINGER OPERATOR EXAMPLE %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figure
plot(imag(u3),'g','LineWidth',1)
ax = gca; ax.FontSize = 14;
hold on
plot(imag(u2),'r','LineWidth',1.5)
plot(imag(u1),'k','LineWidth',2)
...
title('$u^\epsilon(\theta)$','interpreter','latex','fontsize',18)
...
u1 = diffMeas(a,f,X,epsilon,'order',1);
u2 = diffMeas(a,f,X,epsilon,'order',2);
u3 = diffMeas(a,f,X,epsilon,'order',6);

figure
semilogy(X,u1,'k:','linewidth',2)
hold on
semilogy(X,u2,'k--','linewidth',2)
semilogy(X,u3,'k','linewidth',2)
axis([0,2,10^(-8),1])
legend({'$m=1$','$m=2$','$m=6$'},'fontsize',16,'interpreter','latex','location','southeast')
...
% OUTPUTS
% u: chebfun of solutions of linear systems
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

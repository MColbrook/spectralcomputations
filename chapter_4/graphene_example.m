...
%%%%%%%%%%%%%%%% CODE FOR EXAMPLE IN SECTION 4.5.1 %%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% This example computes spectral measure of a magnetic model of graphene.

clear
n=10000; % number of basis sites used
epsilon=0.05; % smoothing parameter
X=-3:0.02:3; % vector of points where we compute the smooth measure
...
H0=T1+T2+T3;
H0=H0(ord,ord)+spdiags(cos(abs(pos(:))*pi)./(1+abs(pos(:))).^2,0,length(ord),length(ord)); % perturbation
...
% perform computation
mu = infmatMeas(H0,b,X,epsilon,'order',6);

...
H0=H0(1:n,1:n); % take rectangular truncation
...
for j=3:N+1
    n_act = j-2;
    a = ((2*n_act+1)/(n_act+1))*H0*v1-n_act*v0/(n_act+1);
    v0 = v1;
    v1 = a;
    MU(j) = a(1)*(2*n_act+3)/2;
    if j==1001
        sum(abs(a)>0)
    end
    if j==2001
...
nu0 = KPMleg(ginf(1:(tt+1)).*MU(1:(tt+1)));
...
for jj=1:length(Nvec)
    ...
    nu1 = KPMleg(MU(1:(Nvec(jj)+1)));
    nu2 = KPMleg(gJ(1:(Nvec(jj)+1)).*MU(1:(Nvec(jj)+1)));
    nu3 = KPMleg(g4(1:(Nvec(jj)+1)).*MU(1:(Nvec(jj)+1)));
    nu4 = KPMleg(ginf(1:(Nvec(jj)+1)).*MU(1:(Nvec(jj)+1)));
...
title('$x_0=0$','interpreter','latex','fontsize',18)
exportgraphics(gcf,'legendre_graphene_KPM_err1.pdf','ContentType','vector','BackgroundColor','none','Colorspace','gray')
...
ax=gca; ax.FontSize=16;
xlabel('$N$','interpreter','latex','fontsize',18)
title('$x_0=0.12$','interpreter','latex','fontsize',18)
exportgraphics(gcf,'legendre_graphene_KPM_err2.pdf','ContentType','vector','BackgroundColor','none','Colorspace','gray')
...
nu1 = KPMleg(MU(1:(N+1)));
nu2 = KPMleg(gJ(1:(N+1)).*MU(1:(N+1)));
nu3 = KPMleg(g4(1:(N+1)).*MU(1:(N+1)));
nu4 = KPMleg(ginf(1:(N+1)).*MU(1:(N+1)));




X= -3:.001:0; % vector of points where we compute the smooth measure
X = X/SC;
...
exportgraphics(gcf,'legendre_graphene_KPM.pdf','ContentType','vector','BackgroundColor','none','Colorspace','gray')



function f = KPMleg(c)

f = chebfun(leg2cheb(real(c)),'coeffs');
end
